...
import { selectAllPicks, selectChallenges, selectLeagues, selectSports, setChallenges } from './features/counterSlice';
...
const Challenge=()=>{
    ...
    const all_picks=useSelector(selectAllPicks);
    ...
    const handle_close_modal=()=>{
      ...
      const load_challenges=async (e)=>{
        const res= db.collection("psg_challenges")
        .orderBy("date","desc")
        .onSnapshot((snap)=>{
            const all_challenge=[];
            snap.docs.map((line)=>{
                    ...
                    <tbody className="tbody_challenge">
                        {
                            challenges.map((challenge)=>{
                                ...
                                const ch_picks=all_picks?.filter((pick)=>{
                                    return pick.id_challenge==challenge.key;
                                })
                                let total_picks=2;
                                if(challenge.type==3 || challenge.type==4){
                                    total_picks=10;
                                }

                                let users_played=ch_picks.map((pick)=>{
                                    return pick.user;
                                })
                                const ch_date=moment(challenge?.date?.seconds*1000).format("ll")

                                const ch_winners=challenge?.winners;
                                console.log("winners are ",ch_winners)
                                return(
                                    <tr key={challenge.key}>
                                        <td>{ch_date}</td>
                                        <td>
                                            {challenge?.results==undefined ? "Battling..." : "Closed"}
                                        </td>
                                        <td align="left">{types[challenge.type]}</td>
                                        ...
                                        <td>{ch_picks.length}/{total_picks}</td>
                                        <td align="left">
                                            <ol>
                                                {
                                                    users_played.map((u,x)=>{
                                                        if(ch_winners!=undefined){
                                                           const res=ch_winners.filter((winner)=>{
                                                            return winner.user==u;
                                                           })
                                                           if(res.length>0){
                                                            return <li key={x} style={{color:"green"}} >{u}</li>
                                                           }else{
                                                            return <li key={x}>{u}</li>
                                                           }
                                                        }else{
                                                            return <li key={x}>{u}</li>
                                                        }
